...
- [ ] 영수증 업데이트 기능
    - [ ] 첫 입력 당시 생성된 구매 응답과 구매 응답에 따른 사용자 응답을 고려하여 영수증을 업데이트 합니다.
        - 추가 수령 요청 응답
        - [ ] 사용자 응답이 `Y`일 경우, 프로모션 적용 횟수를 늘리고 남은 수량을 삭제하여 프로모션 재고에서 판매합니다.
        - [ ] 사용자 응답이 `N`일 경우, 프로모션 적용 수량과 남은 수량 만큼 프로모션 재고에서 판매합니다.
        - 일부 정가 구매 요청 응답
        - [ ] 사용자 응답이 `Y`일 경우, 프로모션 재고에서 우선적으로 판매하고, 남은 수량을 일반 재고에서 판매합니다.
        - [ ] 사용자 응답이 `N`일 경우, 남은 수량을 삭제하고 프로모션 재고에서 판매합니다.
        - 정상 구매 응답
        - [ ] 프로모션 상품 재고가 없을 경우 일반 상품을 판매합니다.
        - [ ] 사용자 응답이 `N`일 경우, 남은 수량을 삭제하고 프로모션 재고에서 판매합니다.
